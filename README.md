...
`ghkit.New` is generic over the returned type; passing `github.NewClient` lets type inference pick up `*github.Client`. ghkit itself has zero compile-time dependency on `go-github` - pass whichever go-github major (or any other `func(*http.Client) T` factory) you want.
...
`WithRequestsPerSecond` is a standard `x/time/rate` token bucket. It adds a client-side cap on top of the reactive limiter, which is useful for batch jobs that want a predictable floor under sustained load.
</details>
...
Credit to bored-engineer. The algorithm walkthrough lives at <https://www.bored-engineer.com/posts/github-etag-algorithm/>.
...
The kit pins `github.com/google/go-github/v85`. To use a different version, build the `*http.Client` with `ghkit.HTTPClient(...)` and hand it to the go-github version you prefer:
